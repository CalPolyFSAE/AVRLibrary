...
uint16_t Transmit_SPI_Master_16(uint8_t Data1, uint8_t Data2) //Function for ATMega to write data to. Must be rewritten to read data from ADC
{
	char receive;
	PORTB &= ~(1 << CS0);			// assert the slave select
...
uint16_t Transmit_SPI_Master(uint8_t Data) //Function for ATMega to write data to. Must be rewritten to read data from ADC
{
	char receive;
	PORTB &= ~(1 << CS0);			// assert the slave select
